...
AS SELECT DISTINCT
*,
NZK_FLG||'@'||KAN_FMP_FLG||'@'||NIGIRI_FLG||'@'||KPRO_FLG||'@'||FAA_FLG||'@'||ZEN_FMP_FLG AS FOCUS_FLG4
FROM FOCUS_JOIN;
DATA PRO_INFO (DROP= NZK_FLG KAN_FMP_FLG NIGIRI_FLG KPRO_FLG FAA_FLG ZEN_FMP_FLG);
SET PRODUCER_INFO;
RUN;
DATA JENN.JEN_HON_PRO3;
SET PRO_INFO;
RUN;
DATA MOMO.PRO_INFO;
SET PRO_INFO;
...
;
data JENN.CROSS_FOCUS; 
set momo.Cross_FOCUS;
run;
PROC EXPORT
DATA=Momo.CROSS_FOCUS
OUTFILE='\\RKDA0050\SME_team\Z_Working_files\Momoko\Focus Group\CROSS_FOCUS.csv' DBMS=csv REPLACE;